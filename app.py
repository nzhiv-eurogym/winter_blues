...
def advice_for_score(total: int) -> tuple[str, str]:
    if 10 <= total <= 19:
        return (
            "🏆 «Ты мой кумир»",
            "- Сон по режиму\n- Дневной свет утром\n- Маленькие радости в планах\n- Лёгкое движение каждый день",
        )
    elif 20 <= total <= 29:
        return (
            "🌤️ «Держись, все будет хорошо!»",
            "- Лечь на 30 минут раньше\n- 10 минут движения\n- 1 маленькое дело в день\n- Таймер на соцсети",
        )
    elif 30 <= total <= 39:
        return (
            "🧣 «Всё не так уж и плохо, как тебе кажется»",
            "- Свет утром 5–10 минут\n- Прогулка слоями + музыка\n- Микро-ритуал (чай + план)\n- Поддержка друга/подруги",
        )
    else:  # 40–50
        return (
            "🤍 «Я с тобой!»",
            "- 1% действия (вода/умыться)\n- Тёплая еда и регулярный перекус\n- День по кусочкам\n- Поговорить с близким взрослым",
        )
